...
module.exports = function (model, privateKeys, protectedKeys) {
  ...
  function findExcludedFieldsFor (fullField, access) {
    ...
    //var path = null
    //
    //// Is this a leaf or full path?
    //currSchema = currSchema.path(fullField).schema
    //if (currSchema) {
    //  path = currSchema.path(fullField)
    //  prefix = fullField
    //} else {
    //  for (var i = 0; i < fieldArray.length - 1; ++i) {
    //    currSchema = currSchema.path(fieldArray[i]).schema
    //  }
    //  path = currSchema.path(fieldArray[fieldArray.length - 1])
